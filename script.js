...
const translateControls = [...document.querySelectorAll("[data-translate-page]")];
const preferredLanguageKey = "preferredSiteLanguage";

function getCanonicalUrl() {
  return document.querySelector('link[rel="canonical"]')?.href || window.location.href;
}

function getArabicTranslateUrl() {
  return `https://translate.google.com/translate?sl=en&tl=ar&u=${encodeURIComponent(getCanonicalUrl())}`;
}

function setTranslationCookie(value, maxAge = 31536000) {
...
function prefersArabic() {
  return localStorage.getItem(preferredLanguageKey) === "ar" || document.cookie.includes("googtrans=/en/ar");
}

function updateTranslateControls() {
  const arabicIsActive = prefersArabic();

  translateControls.forEach((link) => {
    link.href = getArabicTranslateUrl();
    link.classList.add("notranslate");
    link.setAttribute("translate", "no");
    link.setAttribute("aria-pressed", String(arabicIsActive));
    link.textContent = arabicIsActive ? "English" : "العربية";
    link.setAttribute(
      "aria-label",
      arabicIsActive ? "Return to English" : "Translate this site to Arabic"
    );
  });
}

function applyGoogleTranslate(retries = 16) {
  ...
  if (retries > 0) {
    window.setTimeout(() => applyGoogleTranslate(retries - 1), 250);
  }
}

function initializeGoogleTranslate() {
  if (!translateControls.length) return;
  ...
  window.googleTranslateElementInit = () => {
    ...
    if (prefersArabic()) {
      window.setTimeout(() => applyGoogleTranslate(), 400);
    }
  };
...
function enableArabicSiteTranslation() {
  localStorage.setItem(preferredLanguageKey, "ar");
  setTranslationCookie("/en/ar");
  updateTranslateControls();
  initializeGoogleTranslate();
  window.setTimeout(() => applyGoogleTranslate(), 600);
}

function restoreEnglishSite() {
  localStorage.setItem(preferredLanguageKey, "en");
...
if (contactForm) {
  contactForm.addEventListener("submit", async (event) => {
    ...
    const collaboration = String(formData.get("collaboration") || "").trim();
    const message = String(formData.get("message") || "").trim();
    const status = document.querySelector("#contactStatus");
    const submitButton = contactForm.querySelector('button[type="submit"]');
    const endpoint = contactForm.getAttribute("action");
    const fallbackMailto = contactForm.dataset.fallbackMailto || "eng.musaad1@gmail.com";
    const subject = `Website inquiry: ${collaboration || "Collaboration"}`;
    const body = [
      `Name: ${name}`,
      `Email: ${email}`,
    ...
    try {
      ...
      if (status) {
        status.innerHTML = `I could not send through the form service. Please email <a href="mailto:${fallbackMailto}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}">${fallbackMailto}</a> directly.`;
...
function translateAttributes(lang) {
  ...
  document.title = toArabic
    ...
    : "Dr. Musaad Alruwaili | Engineering Knowledge Hub";
  ...
  if (metaDescription) {
    metaDescription.setAttribute(
      ...
      toArabic
        ...
        : "Engineering knowledge hub by Dr. Musaad Alruwaili covering AI, smart grids, SCADA, 5G/6G, IoT, Arduino, research writing, and industry-ready electrical engineering resources."
    );
  }
}
...
const revealTargets = [
  ...document.querySelectorAll(
    ".section, .update-widget, .pillar-grid article, .pathway-card, .research-grid article, .publication-list article, .document-card, .project-card, .experience-card, .education-card, .teaching-grid > div, .blog-grid article, .timeline article, .article-card, .credibility-bar > div, .case-preview"
  )
];
...
const filterableArticles = [...document.querySelectorAll(".filterable-blog article")];
const blogEmpty = document.querySelector("#blogEmpty");

function filterBlogArticles() {
  if (!filterableArticles.length) return;
  ...
  filterableArticles.forEach((article) => {
    const haystack = `${article.textContent} ${article.dataset.tags || ""}`.toLowerCase();
...
if (blogSearch && filterableArticles.length) {
  blogSearch.addEventListener("input", filterBlogArticles);
...
if (tocList) {
  const headings = [...document.querySelectorAll(".article-layout h2, .case-study-content h2")];
  const seenTocLabels = new Set();
  const usedHeadingIds = new Set();

  tocList.textContent = "";
  headings.forEach((heading) => {
    const label = heading.textContent.trim();
    if (!label || seenTocLabels.has(label)) return;

    const baseId = heading.id || label
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "");
    const baseSlug = baseId || "section";
    let headingId = baseSlug;
    let suffix = 2;
    while (usedHeadingIds.has(headingId)) {
      headingId = `${baseSlug}-${suffix}`;
      suffix += 1;
    }

    heading.id = headingId;
    seenTocLabels.add(label);
